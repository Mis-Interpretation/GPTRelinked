...
[CreateAssetMenu(fileName = "TypeChart", menuName = "Scriptable Objects/TurnCombat/TypeChart")]
public class TypeChart : ScriptableObject
{
    #region Editor (Serialized)
    // 18x18 matrix flattened. Row = attacking type, Col = defending type.
    // Values: 0 = immune, 0.5 = not very effective, 1 = normal, 2 = super effective
    [SerializeField] private float[] chart = new float[18 * 18];
    #endregion

    #region Private Variables
    private const int TYPE_COUNT = 18;
    #endregion

    #region Public Functions
    public float GetEffectiveness(ElementType attackType, ElementType defenseType)
    {
        int row = (int)attackType;
        int col = (int)defenseType;
        if (row < 0 || row >= TYPE_COUNT || col < 0 || col >= TYPE_COUNT) return 1f;
        return chart[row * TYPE_COUNT + col];
    }

    public float GetEffectiveness(ElementType attackType, ElementType primaryDef, ElementType secondaryDef, bool hasSecondary)
    {
        float mult = GetEffectiveness(attackType, primaryDef);
        if (hasSecondary)
            mult *= GetEffectiveness(attackType, secondaryDef);
        return mult;
    }

    public void InitializeDefaultChart()
    {
    ...
    private void Set(ElementType atk, ElementType def, float value)
    {
        chart[(int)atk * TYPE_COUNT + (int)def] = value;
    }
    #endregion
}
